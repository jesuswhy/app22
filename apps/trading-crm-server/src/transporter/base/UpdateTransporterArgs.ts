/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TransporterWhereUniqueInput } from "./TransporterWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { TransporterUpdateInput } from "./TransporterUpdateInput";

@ArgsType()
class UpdateTransporterArgs {
  @ApiProperty({
    required: true,
    type: () => TransporterWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => TransporterWhereUniqueInput)
  @Field(() => TransporterWhereUniqueInput, { nullable: false })
  where!: TransporterWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => TransporterUpdateInput,
  })
  @ValidateNested()
  @Type(() => TransporterUpdateInput)
  @Field(() => TransporterUpdateInput, { nullable: false })
  data!: TransporterUpdateInput;
}

export { UpdateTransporterArgs as UpdateTransporterArgs };
